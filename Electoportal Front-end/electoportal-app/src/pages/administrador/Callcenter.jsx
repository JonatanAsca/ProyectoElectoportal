...
const url = "http://localhost:3001/administrador/callcenter/";
...
class Callcenter extends Component {
    ...
    peticionPost = async () => {
        try {
            console.log('Creando callcenter:', this.state.form); // Log para depuración
    ...
    peticionPut = async () => {
        try {
            console.log('Actualizando callcenter:', this.state.form); // Log para depuración
    ...
    peticionDelete = async () => {
        try {
            console.log('Eliminando callcenter:', this.state.form.Cedula); // Log para depuración
    ...
    seleccionarCallcenter = (callcenter) => {
        this.setState({
            tipoModal: 'actualizar',
            form: {
                Cedula: callcenter.Cedula,
                Nombre: callcenter.NombreCallcenter, 
                Apellido: callcenter.Apellido,
                Telefono: callcenter.Telefono,
                Correo: callcenter.Correo,
                Contrasena: callcenter.Contrasena,
                id_administrador: callcenter.id_administrador,
            }
        });
    ...
    handleChange = async e => {
        ...
        if (e.target.name === 'id_administrador') {
            value = value ? parseInt(value, 10) : ''; // Convertir a entero si no está vacío, de lo contrario dr deja vacío
        }
        await this.setState({
            form: {
                ...this.state.form,
    ...
    render() {
        ...
        return (
            <div className='App'>
                <br />
                <button className='btn btn-success' onClick={() => this.modalInsertar()}>Agregar callcenter</button>
                <br /><br />
                <table className='table'>
                    ...
                    <tbody>
                        {this.state.data.map(callcenter => (
                            <tr key={callcenter.Cedula}>
                                <td>{callcenter.Cedula}</td>
                                <td>{callcenter.NombreCallcenter}</td> {/* NombreCallcenter es Nombre */}
                                <td>{callcenter.Apellido}</td>
                                <td>{callcenter.Telefono}</td>
                                <td>{callcenter.Correo}</td>
                                <td>{callcenter.Contrasena}</td>
                                <td>{callcenter.NombreAdministrador}</td>
                                <td>
                                    <button className="btn btn-primary" onClick={() => { this.seleccionarCallcenter(callcenter); }}><FontAwesomeIcon icon={faEdit} /></button>
                                    {"   "}
                                    <button className="btn btn-danger" onClick={() => { this.seleccionarCallcenter(callcenter); this.setState({ modalEliminar: true }) }}><FontAwesomeIcon icon={faTrashAlt} /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                ...
                <Modal isOpen={this.state.modalEliminar}>
                    <ModalBody>
                        Estás seguro que deseas eliminar el callcenter {form && form.Nombre}
                    </ModalBody>
                    <ModalFooter>
